...
public:
    ...
    void CheckCodeSectionIntegrity()
    {
        SectionIntegrity si(mod_libc);
        if (false == si.Init())
        {
            return;
        }
        if (true == si.IsModified(".text"))
        {
            LOG("[%s] .text is corrupted", mod_libc->path.c_str());
        }
        else
        {
            LOG("[%s] .text is healthy", mod_libc->path.c_str());
        }

        if (true == si.IsModified(".rodata"))
        {
            LOG("[%s] .rodata is corrupted", mod_libc->path.c_str());
        }
        else
        {
            LOG("[%s] .rodata is healthy", mod_libc->path.c_str());
        }
        
        SectionIntegrity si2(mod_sample);
        if (false == si2.Init())
        {
            return;
        }
        if (true == si2.IsModified(".text"))
        {
            LOG("[%s] .text is corrupted", mod_sample->path.c_str());
        }
        else
        {
            LOG("[%s] .text is healthy", mod_sample->path.c_str());
        }

        if (true == si2.IsModified(".rodata"))
        {
            LOG("[%s] .rodata is corrupted", mod_sample->path.c_str());
        }
        else
        {
            LOG("[%s] .rodata is healthy", mod_sample->path.c_str());
        }
